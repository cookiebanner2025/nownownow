...
const config = {
    ...
    behavior: {
        ...
        bannerDelay: 0, // Delay in seconds before showing banner (0 = immediate)
...
function loadAnalyticsData() {
    ...
    if (config.analytics.passwordProtect) {
        const authCookie = getCookie('dashboard_auth');
        if (config.analytics.passwordNeverExpire && authCookie === 'true') {
            isDashboardAuthenticated = true;
        } else {
            isDashboardAuthenticated = authCookie === 'true';
        }
    } else {
        isDashboardAuthenticated = true;
    }
...
<div class="cookie-details-content" style="display: none;">
    ...
    <div id="cookieAdminButton" class="cookie-admin-button" title="${lang.dashboardTitle}" style="${config.behavior.adminButtonPosition === 'left' ? 'left: 30px;' : 'right: 30px;'}">
    ...
    const bannerPositionClass = `cookie-consent-banner-${config.behavior.bannerPosition.replace('-', '-')}`;
    ...
    <div id="cookieFloatingButton" class="cookie-settings-button" title="${lang.title}" style="${config.behavior.floatingButtonPosition === 'left' ? 'left: 30px;' : 'right: 30px;'}">
    ...
    <style>
    /* Main Banner Styles - Updated for different positions */
    ...
    .cookie-consent-banner-middle-center {
        top: 50%;
        left: 50%;
        transform: translateX(-50%) translateY(-50%) translateY(20px);
    }

    .cookie-consent-banner-left-middle {
        top: 50%;
        left: 20px;
        transform: translateY(-50%) translateY(20px);
    }

    .cookie-consent-banner-right-middle {
        top: 50%;
        right: 20px;
        transform: translateY(-50%) translateY(20px);
    }
    ...
    .cookie-consent-banner-bottom-center.show {
        transform: translateX(-50%) translateY(0);
    }

    .cookie-consent-banner-middle-center.show {
        transform: translateX(-50%) translateY(-50%) translateY(0);
    }

    .cookie-consent-banner-left-middle.show,
    .cookie-consent-banner-right-middle.show {
