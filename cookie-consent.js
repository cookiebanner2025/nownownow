...
// Add support for programmatic consent changes from other scripts
window.addEventListener('message', function(event) {
    if (event.data.type === 'updateCookieConsent') {
        const validActions = ['accept', 'reject', 'custom'];
            ...
            switch(event.data.action) {
                ...
                case 'custom':
                    if (event.data.categories) {
                        saveCustomSettings(event.data.categories);
                    }
                    break;
            }
        }
...
// Add support for CSP nonce if present
const cspNonce = document.querySelector('script[nonce]')?.nonce || '';
if (cspNonce) {
    document.querySelectorAll('.cookie-consent-banner, .cookie-settings-modal, .cookie-analytics-modal')
        .forEach(el => el.setAttribute('nonce', cspNonce));
}

// Add GDPR compliance check for cookie duration
function validateCookieDurations() {
    const now = new Date();
    Object.entries(cookieDatabase).forEach(([name, details]) => {
        if (details.duration && details.duration !== 'Session') {
            const match = details.duration.match(/(\d+)\s+(day|month|year)/i);
            if (match) {
                ...
                switch(unit) {
                    case 'day': maxDays = value; break;
                    case 'month': maxDays = value * 30; break;
                    case 'year': maxDays = value * 365; break;
                }
                
                if (maxDays > 365) {
                    console.warn(`GDPR Warning: Cookie '${name}' has duration exceeding 1 year (${details.duration})`);
                }
            }
        }
    });
}
validateCookieDurations();

// Add support for document.cookie API interception
const originalCookieSetter = Object.getOwnPropertyDescriptor(Document.prototype, 'cookie').set;
Object.defineProperty(Document.prototype, 'cookie', {
    set: function(value) {
        if (!getCookie('cookie_consent')) {
            const cookieName = value.split('=')[0].trim();
            if (!cookieDatabase[cookieName] && !cookieName.startsWith('_ga') && cookieName !== 'cookie_consent') {
                console.warn('Attempt to set cookie before consent:', cookieName);
                return;
            }
        }
...
if (window.requestIdleCallback) {
    window.requestIdleCallback(() => {
        ...
        const banners = document.querySelectorAll('.cookie-consent-banner:not(.show)');
